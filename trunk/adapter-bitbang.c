 ...
 */
static int bitbang_cpu_stopped (adapter_t *adapter)
{
    bitbang_adapter_t *a = (bitbang_adapter_t*) adapter;
    unsigned ir;

    ir = tap_instr (a, 4, TAP_DEBUG_ENABLE);
    return ir & 4;
}
    ...
    unsigned value, int reg, int reglen)
{
    ...
    if (debug) {
        if (reg == OnCD_OSCR) {
            ...
            if (reg & IRd_RESUME)     fprintf (stderr, "+RESUME");
 ...
 */
static void bitbang_reset_cpu (adapter_t *adapter)
{
    bitbang_adapter_t *a = (bitbang_adapter_t*) adapter;
    unsigned idle = NTRST | NSYSRST | TMS | TDI;

    /* Активируем /SYSRST на 5 микросекунд. */
    bitbang_send_recv (a);
    a->output [a->output_len++] = idle & ~NSYSRST;
    a->output [a->output_len++] = idle & ~NSYSRST;
    a->output [a->output_len++] = idle & ~NSYSRST;
    a->output [a->output_len++] = idle & ~NSYSRST;
    a->output [a->output_len++] = idle & ~NSYSRST;
    a->output [a->output_len++] = idle;
    bitbang_send_recv (a);
}
...
failed:     usb_release_interface (a->usbdev, 0);
    ...
    a->adapter.cpu_stopped = bitbang_cpu_stopped;
    a->adapter.stop_cpu = bitbang_stop_cpu;
    a->adapter.reset_cpu = bitbang_reset_cpu;
